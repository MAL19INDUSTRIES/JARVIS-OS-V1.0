...
def _resolve_save_path(output_path: str, language: str) -> Path:
    ...
    if output_path:
        ...
        is_website = language_key in {"html", "htm"} or p.suffix.lower() in {".html", ".htm"}
        base = WEBSITES_DIR if is_website else DESKTOP
        return p if p.is_absolute() else base / p
    ext = ext_map.get((language or "python").lower(), ".py")
    if ext in {".html", ".htm"}:
        return WEBSITES_DIR / "jarvis_website" / "index.html"
...
def _notify_html_preview(player, path: Path, persona: str = "JARVIS") -> bool:
    """Open authored HTML in the integrated UI when that surface is available."""
    target = Path(path)
    callback = getattr(player, "show_website_preview", None)
    if target.suffix.lower() not in {".html", ".htm"} or not callable(callback):
        return False
    callback(str(target.resolve()), str(persona or "JARVIS").upper())
    return True
...
def _build(
    ...
    try:
        ...
        _notify_html_preview(player, path, persona)
    except Exception as e:
...
def _write_action(description, language, output_path, player, persona="JARVIS") -> str:
    if not description:
...
def _edit_action(file_path, instruction, player, persona="JARVIS") -> str:
    if not file_path:
...
Updated code:"""
    ...
    _notify_html_preview(player, Path(file_path), persona)
...
def _run_action(file_path, args, timeout, player, persona="JARVIS") -> str:
    if not file_path:
    ...
    if _notify_html_preview(player, p, persona):
        return f"Website preview opened for {p}."
    return _run_file(p, args, timeout)


def _optimize_action(
    file_path, code, language, output_path, player, persona="JARVIS"
) -> str:
...
Optimized code:"""
    ...
    _notify_html_preview(player, save_path, persona)
...
def code_helper(
    ...
    if action == "write":
        return _write_action(description, language, output_path, player, persona)
        ...
        return _run_action(file_path, args, timeout, player, persona)

    elif action == "build":
        return _build(
            description, language, output_path, args, timeout,
            speak, player, persona,
        )

    elif action == "optimize":
        return _optimize_action(
            file_path, code, language, output_path, player, persona
        )
